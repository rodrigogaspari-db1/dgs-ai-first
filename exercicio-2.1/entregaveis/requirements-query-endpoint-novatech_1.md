...
**Bounded contexts envolvidos:** BC-1 (Atendimento ao Cliente) e BC-2 (Curadoria e Confiabilidade do Conhecimento), conforme o Recorte de Domínio e Bounded Contexts v2.0.
...
Derivados do recorte de domínio: este módulo é o ponto de tradução entre o BC-2 (que decide qual fonte tem autoridade e qual o nível de confiança) e o BC-1 (que consome essa decisão já pronta para conduzir o atendimento).
...
- Aplicar o filtro de confidencialidade por perfil do solicitante antes de a resposta ser entregue — consumindo a classificação já definida pela Governança (BC-3), sem redecidi-la.
- Preservar continuidade de sentido dentro de uma mesma sessão de atendimento sem permitir que perguntas anteriores contaminem ou substituam a base documental relevante para a pergunta atual.
...
| Recorte de Domínio e Bounded Contexts v2.0 | Seis bounded contexts do domínio de logística/atendimento. | Este módulo atravessa BC-1 e BC-2, conforme a Seção 2 (Scope Boundaries) acima — coerente com o mapeamento já feito na Seção 6 daquele documento. |
...
Critérios objetivos, verificáveis pelo QA sem depender de interpretação pessoal — usando o mapa de cobertura do Anexo B como massa de teste de referência.

**VC-01 — Tempo de resposta.** Em uma amostra de perguntas do mapa de cobertura do Anexo B, o tempo de processamento deste módulo (da pergunta recebida até a resposta pronta para exibição) permite cumprir a meta fim-a-fim de 30 segundos (C1) em pelo menos 95% dos casos.
...
**VC-07 — Indistinguibilidade entre "restrito" e "não encontrado".** Comparando a resposta a uma pergunta cuja única fonte é confidencial e não autorizada para o perfil do solicitante com a resposta a uma pergunta genuinamente sem cobertura, as duas são idênticas em conteúdo e sem diferença perceptível de tempo de resposta — qualquer diferença detectável reprova o caso.
...
**VC-10 — Continuidade sem degradação.** Numa sequência de 5 perguntas de temas diferentes na mesma sessão, a 5ª pergunta é avaliada com a mesma precisão que teria como 1ª pergunta da sessão — uma queda de precisão atribuível à posição na sequência (não ao conteúdo da pergunta em si) reprova o caso.
