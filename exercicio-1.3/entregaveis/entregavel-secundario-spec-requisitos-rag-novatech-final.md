...
> **Versão 2.0 — Final (testabilidade consolidada).** Esta versão incorpora os critérios de teste adicionados ou reforçados na revisão de QA sobre a v1.0 (23 dos 33 requisitos ganharam critério novo ou mais preciso; ver seção de Convenções para o formato mínimo assumido). Pontos que ainda dependem de uma decisão de negócio do cliente ou do Tech Lead estão marcados como **[a validar]** e também listados na Seção 8.
...
*Critério de teste:* uma resposta originada só no FAQ não-validado sempre carrega o prefixo `[NÃO OFICIAL]` (ver Convenções) e nível de confiança Baixa ou Média — nunca Alta; uma resposta equivalente baseada em POL-001 ou SLA-2024 não carrega esse prefixo e pode atingir Alta. Testável comparando o texto literal das duas respostas (ver REQ-08).
...
**Regra de desempate:** um documento `sujeito_a_revisao` (REQ-03) sempre rebaixa a confiança da resposta para, no máximo, Média — mesmo quando não há conflito de versão nem problema de vigência — porque o conteúdo pode mudar em breve. Isso resolve a sobreposição entre o aviso do REQ-03 e o nível de confiança deste requisito.

*Critério de teste:* dado o mesmo conjunto de chunks recuperados, dois avaliadores de QA aplicando a regra (incluindo a regra de desempate acima) chegam ao mesmo nível de confiança em pelo menos 10 combinações de teste cobrindo cada nível e o caso de sobreposição com REQ-03.
...
- (b) **Resposta genérica sinalizada:** permitida apenas para perguntas de conhecimento geral não específico da NovaTech (ex.: explicar o que é um CT-e enquanto conceito de mercado, não uma regra interna) — sempre com o prefixo `[RESPOSTA GENÉRICA — não baseada em documentação da NovaTech, verifique antes de repassar ao cliente]` (ver Convenções).

*Regra operacional para a fronteira do caso (b):* considera-se "conhecimento geral" apenas conteúdo que não menciona nenhuma entidade específica da NovaTech (nome de documento, valor, prazo, tier, região, cláusula) nem decisão operacional da empresa — qualquer menção a esses elementos exige fonte documental, nunca resposta genérica. **[Regra proposta como ponto de partida — confirmar com o cliente, ver ponto em aberto 4, Seção 8, dado o risco jurídico já levantado no discovery.]**

*Critério de teste:* nenhuma resposta contendo valor numérico, prazo, percentual ou regra de negócio específica é entregue sem citação de fonte documental; qualquer resposta sem essa citação carrega obrigatoriamente o prefixo `[RESPOSTA GENÉRICA]`.
...
**Requisito de suporte:** para que o filtro seja verificável, o sistema expõe, para cada resposta, um campo estruturado de citações (lista de fontes usadas) separado do texto livre da resposta — o filtro determinístico verifica esse campo, não o texto da resposta.

*Critério de teste:* rodando um conjunto de perguntas sem correspondência na base (ex.: perguntas de conhecimento geral de mercado), 100% das respostas resultantes carregam o prefixo `[RESPOSTA GENÉRICA]`, verificável a partir do campo estruturado de citações vazio — sem exceção, independentemente da fraseação usada pelo modelo.
...
*Critério de teste:* uma amostra de 100% dos documentos indexados (incluindo os já existentes hoje, retroativamente) contém os 4 campos preenchidos; documento sem algum desses campos nunca aparece como fonte com nível de confiança Alta (cai automaticamente para Baixa, por REQ-08).
...
*Critério de teste:* em teste de usabilidade, o número de cliques/toques necessários para visualizar o trecho original a partir da resposta no Teams é igual a 1, sem sair da conversa ativa com o cliente.
...
*Critério de teste:* uma amostra de 100% das interações do período de teste gera registro de auditoria com os 5 campos obrigatórios (pergunta, resposta, fonte(s) citada(s), nível de confiança, indicação de sinalização de erro); interação sem registro completo reprova o caso de teste.
...
*Critério de teste:* ao sinalizar uma resposta como incorreta, o registro de auditoria da interação original é atualizado/vinculado com a sinalização, o motivo e o atendente responsável; o status de confiança do trecho correspondente muda para refletir a sinalização pendente, verificável consultando o mesmo trecho a partir de outro atendente antes da resolução.
...
3. Validar limiares exatos de confiança (Alta/Média/Baixa) com os donos de cada fonte, incluindo o SLA de revisão periódica (REQ-15).
...
9. Confirmar frequência e método do processo de auditoria periódica de contradições entre documentos correlatos (REQ-09) — proposta: mensal, a validar.
...
*Critério de teste:* existe documento formal nomeando a área responsável pela manutenção do banco vetorial, com meta de disponibilidade mínima explícita — recomenda-se herdar o piso já contratual do SLA-2024 (99,5% para compromissos equivalentes ao tier Gold) — medida mensalmente e comparada à meta definida.
...
*Critério de teste:* uma amostra de 100% das tentativas de acesso negado durante o período de teste gera entrada na trilha de auditoria com usuário, chunk restrito e motivo; tentativa sem registro reprova o caso de teste.
...
*Critério de teste:* uma amostra de 100% dos chunks armazenados contém os 4 níveis de metadado hierárquico preenchidos (área > pasta > documento > seção); chunk sem essa cadeia completa reprova o caso de teste.
...
*Critério de teste:* ao publicar um novo documento em uma pasta/área já mapeada no comitê de governança, o dono responsável é atribuído automaticamente ao registro do documento, sem cadastro manual — verificável comparando o dono atribuído ao mapeamento oficial da Seção 6.

---

## 12. Sumário de rastreabilidade (requisito → tipo de verificação)

Para orientar o plano de testes do projeto (a ser detalhado em exercício futuro), os 33 requisitos se dividem em três naturezas de verificação:

- **Teste de sistema/funcional, automatizável contra o mapa de cobertura do Anexo B:** REQ-01 a REQ-12, REQ-16 a REQ-18, REQ-25 a REQ-27, REQ-30 a REQ-33.
- **Auditoria de processo/organizacional (verifica-se a existência de comitê, relatório ou documento, não uma resposta do assistente):** REQ-15, REQ-19 a REQ-21, REQ-28, REQ-29.
- **Teste de infraestrutura/operação (requer ambiente de homologação):** REQ-13, REQ-14, REQ-22 a REQ-24, REQ-31.

Todos os 33 requisitos têm hoje um critério de teste objetivo; 6 deles (REQ-09, REQ-15, REQ-24, REQ-26, REQ-32, e o caso (b) do REQ-10) permanecem **condicionalmente testáveis**, dependendo de uma decisão de negócio ainda listada na Seção 8.
